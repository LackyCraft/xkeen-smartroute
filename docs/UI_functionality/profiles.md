...
Кнопка «Удалить» — с подтверждением (диалог с именем профиля), тем же
паттерном, что и удаление подписки/списка доменов: одно случайное
нажатие раньше могло стереть профиль (его доменную маршрутизацию,
выбор серверов, участие в kill-switch) без возможности отменить.
Если удаление реально не удалось на backend — появляется сообщение с
настоящей причиной, профиль остаётся в списке.
