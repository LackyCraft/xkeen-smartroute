...
## Если переключатель не применился

Каждый из четырёх тумблеров может реально не примениться (например,
сгенерированные nftables-правила не прошли собственную проверку
синтаксиса firewall) — в этом случае тумблер возвращается в прежнее
положение, и появляется сообщение с настоящей причиной сбоя, а не
тихо остаётся в состоянии, не соответствующем реальному состоянию
firewall (раньше — реальный найденный баг — панель всегда сообщала об
успехе независимо от исхода). Прошлая работающая конфигурация firewall
при этом не теряется — она восстанавливается, а не удаляется.
