...
- **Kill-switch без зазора по времени** — постоянно armed firewall-правило, не polling раз в минуту.
...
| Панель на порту 1001 не открывается | `pgrep -f smartroute-gateway`; пусто — `/opt/etc/init.d/S98smartroute-gateway restart`, лог — `/etc/xkeen-smartroute/state/gateway.log` |
