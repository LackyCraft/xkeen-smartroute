...
- Free space: `install.sh` will start with **~25 MB** free on overlay, but that's cutting it close. Measured on a real test router (92 MB overlay): the Xray binary — 32 MB, geosite/geoip databases — 19 MB, xkeen-UI — 7 MB, the whole SmartRoute module (lib scripts, the `smartroute-gateway` panel, domain lists) — 33 MB — **~90 MB** total for the stack alone, before subscription growth or logs. If your router's flash doesn't have that, plug in a **USB stick** and put Entware there (standard practice; the Entware installer will offer to pick a partition).
